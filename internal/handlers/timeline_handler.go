...
type timelineRequest struct {
	...
	IsPublic    pgtype.Bool `json:"is_public,omitempty"`
}
...
func (th *TimelineHandler) HandleCreateTimeline(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		th.logger.Printf("Invalid user ID: %v", err)
...
func (th *TimelineHandler) HandleGetPublicTimelines(w http.ResponseWriter, r *http.Request) {
	...
	timelines, err := th.timelineStore.GetPublicTimelines(r.Context(), userID)
	if err != nil {
		th.logger.Printf("Failed to retrieve timeline %v", err)
		utils.WriteJSON(w, http.StatusInternalServerError, utils.Envelope{"message": "Failed to retrieve timeline"})
		return
...
func (th *TimelineHandler) HandleSearchPublicTimeline(w http.ResponseWriter, r *http.Request) {
	title := r.URL.Query().Get("title")
	userID, err := utils.ReadUserIDFromContext(r, "userID")
	if err != nil {
		th.logger.Printf("Invalid user ID: %v", err)
	...
	timelines, err := th.timelineStore.GetPublicTimelinesByTitle(r.Context(), db.GetPublicTimelinesByTitleParams{
		UserID:  userID,
...
func (th *TimelineHandler) HandleUpdateTimeline(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		if err.Error() == "no rows in result set" {
			utils.WriteJSON(w, http.StatusNotFound, utils.Envelope{"message": "You're not allowed to update this timeline"})
			return
		}
...
func (th *TimelineHandler) HandleDeleteTimeline(w http.ResponseWriter, r *http.Request) {
	...
	rowsAffected, err := th.timelineStore.DeleteTimeline(r.Context(), db.DeleteTimelineParams{
		ID:     timelineID,
		UserID: userID,
	})
	if err != nil {
	...
	if rowsAffected == 0 {
		utils.WriteJSON(w, http.StatusForbidden, utils.Envelope{
			"message": "You are not allowed to delete this timeline",
		})
		return
	}
